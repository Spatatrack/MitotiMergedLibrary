...
//  3DButton
//
//  Created by Simone Pistecchia on 03/10/2020.
//

import SwiftUI

///holdOnPress if true, remain down if active
struct IperButton: View {

    var systemImageName: String = "mic.fill"
    var holdOnPress: Bool = false
    
    @State private var opacity = 1.0
    @State private var logoScale: CGFloat = 1 //on pressure
    ...
    //Color(  colorLiteral(red: 0.565, green: 0.608, blue: 0.667, alpha:1)  basta mettere # prima di colorLiteral e si trasforma in colore
    //ICON
    let darkGradient = Gradient(colors: [
                                    Color(#colorLiteral(red: 0.192, green: 0.212, blue: 0.329, alpha:1)),
                                    Color(#colorLiteral(red: 0.565, green: 0.608, blue: 0.667, alpha:1))])
    let redGradient = Gradient(colors:[
                                Color(#colorLiteral(red: 0.51, green: 0.22, blue: 0.22, alpha:1)),
                                Color(#colorLiteral(red: 0.81, green: 0.22, blue: 0.22, alpha:1))])
    
    let offWhite = Color(#colorLiteral(red: 0.921431005, green: 0.9214526415, blue: 0.9214410186, alpha: 1))//Color(#colorLiteral(red: 225 / 255, green: 225 / 255, blue: 235 / 255, alpha:1))
        ...
        GeometryReader { dim in
            ZStack {
                ZStack {
                    ...
                    Circle()
                        .fill(offWhite)
                        .frame(width: dim.size.width, height: dim.size.height)
                    ...
                    .mask(
                        Circle().frame(width: dim.size.width, height: dim.size.height)
                            .scaleEffect(self.logoScale)
                    )
                    
                    // logo
                    LinearGradient(
                        gradient: self.buttonOn ? redGradient : darkGradient,
                      startPoint: .leading,
                      endPoint: .trailing
                    )
                    .scaleEffect(self.logoScale)
                ...
                .onTapGesture {
                    ...
                    withAnimation(Animation.easeIn(duration: 0.15)) {
                        ...
                        self.logoScale = 0.9 // -10%
                    }
                    withAnimation(Animation.easeOut(duration: 0.15)) {
                        self.shadowOpacity12 = 0.2
                        self.shadowOpacity22 = 0.7
                        self.logoScale = 0.8 // -20%
                    }
                    if holdOnPress == false {
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                            ...
                            withAnimation(Animation.easeOut(duration: 0.15)) {
                                ...
                                self.logoScale = 1 // restore original
                            }
                        }
                    }
                    else {
                    ...
                    self.buttonOn.toggle()
                    if self.buttonOn && !self.animating {
                        self.opacity = 1
...
struct IperButton_Previews: PreviewProvider {
